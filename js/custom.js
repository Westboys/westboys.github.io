...
(function ($) {
    ...
    $('#iniContacto').click(function () {
        ...
        
        {
            form.css({opacity: 1});
            ajaxLoad.hide();
            $(formMessages).removeClass('msg_success');
            $(formMessages).addClass('msg_error');
            $(formMessages).text("Favor de capturar el nombre del contacto.");
            return;
        }
        ...
        if ( $('#email').val().length <= 0)
        {
            ...
            $(formMessages).removeClass('msg_success');
            $(formMessages).addClass('msg_error');
            $(formMessages).text("Favor de capturar un correo.");
            return;
        }
        ...
        if (regex.test($('#email').val().trim()) == false ) {
            ...
            $(formMessages).removeClass('msg_success');
            $(formMessages).addClass('msg_error');
            $(formMessages).text("Favor de capturar un correo valido, (ejemplo: micorreo@empresa.com)");
            return;
        }
        ...
        if ( $('#subject').val().length <= 0)
        {
            ...
            $(formMessages).removeClass('msg_success');
            $(formMessages).addClass('msg_error');
            $(formMessages).text("Favor de capturar el titulo del mensaje");
            return;
        }
        if ( $('#message').val().length <= 0)
        {
            ...
            $(formMessages).removeClass('msg_success');
            $(formMessages).addClass('msg_error');
            $(formMessages).text("Favor de capturar el mensaje detallado");
            return;
        }

        var strData = $('#name').val() + "|" + $('#email').val() + "|" +$('#subject').val() + "|"+ $('#message').val();
        
        var arForm = form.serializeArray();
        $.ajax({
            type: 'POST',
            url: '/contact.asmx/cotizacion',
            //data: "{'data':'" + strData + "'}", 
            ...
            success: function (data) {
                if (data.d != null) {
                    ...
                    form.css({opacity: 1});
                    ajaxLoad.hide();
                    $(formMessages).removeClass('msg_error');
                    $(formMessages).addClass('msg_success');
                    $(formMessages).text(data.d.Mensaje);
                }
            },
            error: function (jqXHR, textStatus, errorThrown) {
                ...
                $(formMessages).removeClass('msg_success');
                $(formMessages).addClass('msg_error');
                $(formMessages).text("Ocurrio el siguiente error: " + errorThrown);
            }
        }); 
    ...
    /******************************************************************************************************************/
    /** AJAX ACCESO CON CLAVE FORM C#                                                                                       */
    /******************************************************************************************************************/
    // jQuery Validation
    $('#IniSesion').click(function () {

        $('#subscribes-response').html("");
        if ( $('#pass').val().length > 0)
        {
            $('#subscribes-response').html("Procesando, por favor espere...");
            $.ajax({
                type: 'POST',
                url: '/contact.asmx/descarga',
                data: "{'data':'" + $("#pass").val() + "'}", 
                contentType: 'application/json; utf-8',
                dataType: 'json',
                success: function (data) {
                    if (data.d != null) {
                        $('#pass').val("");
                        $('#subscribes-response').html( (data.d.Resultado=='OK') ? 'Contraseña Correcta' : 'Contraseña Incorrecta');
                        if (data.d.Resultado=="OK") { window.location.href = "download.html"; }
                    }
                },
                error: function (jqXHR, textStatus, errorThrown) {}
            }); 
        ...
        { 
            $('#subscribes-response').html("Favor de capturar la contraseña");
        }
    });
    

    /******************************************************************************************************************/
    /** AJAX SUBSCRIBE FORM C#                                                                                       */
    /******************************************************************************************************************/
    // jQuery Validation
    $('#IniSubscribe').click(function () {
        $('#subscribe-response').html("");
        if ( $('#subscribe_email').val().length > 0)
        {
            $('#subscribe-response').html("Procesando, por favor espere...");
            $.ajax({
                type: 'POST',
                url: '/contact.asmx/subscribe',
                data: "{'data':'" + $("#subscribe_email").val() + "'}", 
                contentType: 'application/json; utf-8',
                dataType: 'json',
                success: function (data) {
                    if (data.d != null) {
                        $('#subscribe-response').html(data.d.Mensaje);
                        $('#subscribe_email').val("");
                    }
                },
                error: function (jqXHR, textStatus, errorThrown) {}
        ...
        { 
            $('#subscribe-response').html("Favor de capturar un correo valido.");
        }
    });
